...
"""GR1 robot-policy posttraining recipe v2 for Cosmos3-Nano.

Changes from v1 (gr1_robot_policy_posttrain):
  - rectified_flow loss_scale = 10.0  (balance vision vs action loss)
  - encode_exact_durations = [17]     (pin VAE encode to chunk_length+1 frames)
  - Image augmentation (random crop 95% + color jitter, matching DROID recipe)
"""
...
class GR1LeRobotDatasetV2(GR1LeRobotDataset):
    """GR1 dataset with optional image augmentation (matching DROID recipe).

    When ``use_image_augmentation=True``, applies random crop (95%) + resize +
    color jitter uniformly across all frames. The augmentor is lazy-built on the
    first call and resampled per ``__getitem__`` (temporally consistent within
    one sample, different across samples).
    """
...
class GR1RobotPolicyDataPackerV2(DataPacker):
    """Action policy DataPacker for GR1 LeRobot samples (v2)."""
...
gr1_robot_policy_posttrain_v2 = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            project="cosmos3",
            group="gr1_robot_policy_v2",
            name="gr1_robot_policy_posttrain_v2_${now:%Y-%m-%d}_${now:%H-%M-%S}",
        ...
        dataloader_train=L(DataPackerDataLoader)(
            data_source=L(GR1LeRobotDatasetV2)(
            ...
            data_packer=L(GR1RobotPolicyDataPackerV2)(
...
# --- v2 changes from DROID recipe ---

# Pin VAE encode duration to match chunk_length + 1 = 17 observation frames.
gr1_robot_policy_posttrain_v2["model"]["config"]["tokenizer"]["encode_exact_durations"] = [17]

# Balance vision flow-matching loss against action loss (action_loss_weight=10).
# Without this, vision loss is 10x weaker → degraded visual features → worse action prediction.
gr1_robot_policy_posttrain_v2["model"]["config"]["rectified_flow_training_config"]["loss_scale"] = 10.0


cs.store(group="experiment", package="_global_", name="gr1_robot_policy_posttrain_v2", node=gr1_robot_policy_posttrain_v2)
