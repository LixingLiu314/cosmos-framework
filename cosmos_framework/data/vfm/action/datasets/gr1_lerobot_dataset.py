...
class GR1LeRobotDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        raw_state = self._extract_modal_vector(observation_rows[:1], self._state_key, self._state_parts)
        task = self._caption_for(observation_rows[0], episode)

        state = self._normalize_state_tensor(raw_state) if self._normalize_state else raw_state
        extras: dict[str, Any] = {"state": state}
        return self._build_result(mode=mode, video=video, action=raw_action, ai_caption=task, **extras)
    ...
    def _load_video(self, episode: dict[str, Any], observation_rows: list[dict[str, Any]]) -> torch.Tensor:
        ...
        return decode_video_frames(self._video_path(episode, self._video_key), frame_timestamps, self._tolerance_s)
    ...
    def _normalize_state_tensor(self, state: torch.Tensor) -> torch.Tensor:
        stats = self._load_state_stats()
        return normalize_action(state, "minmax", stats)
